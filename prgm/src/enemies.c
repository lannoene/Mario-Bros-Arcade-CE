...
void UpdateEnemies(player_t* player, unsigned int gameFrame) {
	for (uint8_t i = 0; i < levelEnemies.numEnemies; i++) {
		...
		switch (levelEnemies.enemyArray[i].type) {
			case ENEMY_SPIKE:
				// fallthrough
			case ENEMY_CRAB:
				CalcForSpinies(player, gameFrame, i);
				break;
			case ENEMY_FLY:
	...
	/*
	for (uint8_t i = 0; i < levelEnemies.numEnemies; i++) {
		for (uint8_t j = 0; j < levelEnemies.numEnemies/2 + 1; j++) { // causes an enormous amount of lag
			if (j != i && levelEnemies.enemyArray[j].y + ENEMY_SPIKE_HITBOX_HEIGHT > levelEnemies.enemyArray[i].y && levelEnemies.enemyArray[j].y < levelEnemies.enemyArray[i].y + ENEMY_SPIKE_HITBOX_HEIGHT && levelEnemies.enemyArray[j].x + ENEMY_SPIKE_SIZE > levelEnemies.enemyArray[i].x && levelEnemies.enemyArray[j].x < levelEnemies.enemyArray[i].x + ENEMY_SPIKE_SIZE) {
				if (levelEnemies.enemyArray[i].state == ENEMY_WALKING)
					levelEnemies.enemyArray[i].dir = (levelEnemies.enemyArray[i].dir == LEFT) ? RIGHT : LEFT;
				if (levelEnemies.enemyArray[j].state == ENEMY_WALKING)
					levelEnemies.enemyArray[j].dir = (levelEnemies.enemyArray[j].dir == LEFT) ? RIGHT : LEFT;
			}
		}
	}*/
}
	...
	switch (levelEnemies.enemyArray[i].state) {
		case ENEMY_WALKING:
			if (levelEnemies.enemyArray[i].dir == LEFT &&levelEnemies.enemyArray[i].grounded == true && levelEnemies.enemyArray[i].verAccel <= 0)
				levelEnemies.enemyArray[i].horAccel = -levelEnemies.enemyArray[i].maxSpeed - ((levelEnemies.enemyArray[i].crabIsMad) ? 0.25 : 0);
